class Scene extends GameScene {
    constructor(game) {
        ...
        this.backgroundSystem = {
            ...
            draw: function (context, map) {
                ...
                // 计算第一张图的x位置
                const firstImageX =
                    Math.floor(bgOffset / this.width) * this.width;

                // 绘制多张背景图片实现无限滚动
                for (let i = 0; i < repeatCount; i++) {
                    const x =
                        firstImageX + i * this.width + (bgOffset % this.width);
                    context.drawImage(
                        this.texture,
                        x,
                        0,
        ...
        // 创建宠物并跟随玩家
        let pet = new Pet(game, player, map);
        this.pet = pet;
        // 将宠物添加到场景中
        this.addElement(pet);
    ...
    update() {
        ...
        const xCollision =
            xDistance < (playerHitboxWidth + enemyHitboxWidth) / 2;
        ...
        const yCenterDistance = Math.abs(
            playerY + this.player.h / 2 - (enemyY + this.enemy.h / 2),
        );
        const yProximity = yCenterDistance < yProximityThreshold;
        const yAttackRange =
            yCenterDistance < yAttackThreshold && onSamePlatform;
    ...
    draw() {
        ...
        if (this.debugModule) {
            ...
            // 绘制宠物碰撞
            this.debugModule.drawPetCollision(this.pet);
