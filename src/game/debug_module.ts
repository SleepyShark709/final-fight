...
import { GameTileMap } from "./game_map";
...
export class DebugModule {
  ...
  // 绘制玩家碰撞盒
  drawPlayerCollision(player: Player) {
  ...
  // 绘制敌人碰撞盒
  drawEnemyCollision(enemy: Enemy) {
  ...
  // 绘制地图瓦片碰撞可视化
  drawMapCollision(map: GameTileMap) {
  ...
  // 绘制摄像机调试信息
  drawCameraDebug(map: GameTileMap) {
    if (!this.enabled || !map) return;
