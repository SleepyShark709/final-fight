...
export class HpBar {
  ...
  update(percentage: number) {
    ...
    this.redW =
      this.w - this.w * this.percentage > this.w
        ? this.w
        : this.w - this.w * this.percentage; // 当红色条的宽度大于默认宽的时候，让其为默认宽
  }
  ...
  draw() {
    ...
    this.game.context.fillRect(
      screenX + this.w * this.percentage < screenX
        ? screenX
        : screenX + this.w * this.percentage,
      screenY,
      this.redW,
      10
    );
