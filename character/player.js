...
class Player extends Character {
    ...
    updateGravity() {
        ...
        if (this.isOnGround) {
            ...
            if (this.vy > 0) {
                ...
                this.vy = 0;
                console.log(
                    `落地调整位置: 从${oldY}到${this.y}, vy设为${this.vy}`,
                );
                ...
                this.vy = 0;
                console.log(
                    `低速落地调整: 从${oldY}到${this.y}, vy从${this.vy}设为0`,
                );
            }
            ...
            if (this.isJump && this.jumpHitWall && this.vy < 0) {
                ...
                const centerX = Math.floor(
                    (this.x + this.w / 2) / this.tileSize,
                );
                const leftWall = this.map.onTheGround(
                    centerX - 1,
                    playerGridY + 1,
                );
                const rightWall = this.map.onTheGround(
                    centerX + 1,
                    playerGridY + 1,
                );
    ...
    checkHorizontalCollision() {
        ...
        if (!this.isOnGround) {
            checkPoints.push(
                Math.floor((this.y + this.h * 0.7) / this.tileSize),
            ); // 下部位置
        }
    ...
    attack(enemy, scene) {
        ...
        if (this.cooldown === 0) {
            ...
            this.frameCount =
                PLAYER_ATTACK_TYPE_1_OR_2_NUMBER * RENDER_IMAGE_NUMBER;
            this.isAttack = true;
            ...
            if (
                this.x > enemy.x + 30 - this.w &&
                this.x < enemy.x + enemy.w - 30
            ) {
                console.log('开始攻击');
    ...
    move(x, keyStatus) {
        ...
        if (this.isJump && this.vy < 0) {
            ...
            if (x < 0) {
                ...
                const leftCheckX = centerX - 1;
                // 检查当前高度是否有墙，下一格是否有墙
                const hasWallCurrent = this.map.onTheGround(
                    leftCheckX,
                    playerTopY,
                );
                const hasWallBelow = this.map.onTheGround(
                    leftCheckX,
                    playerTopY + 1,
                );
